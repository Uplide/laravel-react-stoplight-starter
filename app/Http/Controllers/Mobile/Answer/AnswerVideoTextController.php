...
use App\Http\Requests\Mobile\Answer\AnswerVideoTextRequest;
 ...
 */
class AnswerVideoTextController extends Controller
{

    public $prefix = "answers/videos/";

    /**
     * Create Video & Text Answer
     *
     * Bu servis soru tipi video & metin  ise kullanılması gereken servistir.
     *
     * @requestMediaType multipart/form-data
     *
     */
    public function create(AnswerVideoTextRequest $request)
    {
        ...
        if ($question->option_type !== QuestionTypes::VIDEO->value) {
            abort(response()->json([
                "message" => "Sadece video tipindeki sorular için cevap gönderilebilir."
            ], 400));
        }
        ...
        if ($isUserRelated) {
            ...
            $filePath = uniqid() . Helper::makeId(20) . "." . $request->video->getClientOriginalExtension();

            $destinationPath = public_path($this->prefix);
            $request->video->move($destinationPath, $filePath);

            $userQuestionAnswer = UserQuestionAnswer::create([
                "user_id" => $userId,
                "video_path" => env("APP_URL") . "/" . $this->prefix . $filePath,
     ...
     *
     */
    public function update(AnswerVideoTextRequest $request)
    {
        $userId = null;
        $questionId = $request->id;
        if (!$userId = @(Auth::guard('user-api')->user())->id) {
        ...
        $getFileName = Helper::getFileName(
            ...
            newExtension: $request->video->getClientOriginalExtension(),
            prefix: $this->prefix
        );
        ...
        $destinationPath = public_path($this->prefix);
        $request->video->move($destinationPath, $getFileName["fileNameNewExtension"]);

        $userQuestionAnswer = $answer->update([
            "video_path" =>  env("APP_URL") . "/" . $this->prefix . $getFileName["fileNameNewExtension"],
            "is_editable" => false,
            "text_answer" => $request->text_answer,
        ]);
