...
use App\Http\Requests\Mobile\Answer\AnswerImageRequest;
...
class AnswerImageController extends Controller
{
     ...
     * Create Image Answer
     *
     * Bu servis soru tipi resim ise kullanılması gereken servistir.
     *
     * @requestMediaType multipart/form-data
     *
     */
    public function create(AnswerImageRequest $request)
    {
        ...
        if ($question->option_type !== QuestionTypes::IMAGE->value) {
            abort(response()->json([
                "message" => "Sadece resim tipindeki sorular için cevap gönderilebilir."
            ], 400));
        }
     ...
     *
     */
    public function update(AnswerImageRequest $request)
    {
        $userId = null;
        $questionId = $request->id;
        if (!$userId = @(Auth::guard('user-api')->user())->id) {
