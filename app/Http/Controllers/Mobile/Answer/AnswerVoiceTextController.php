...
use App\Http\Requests\Mobile\Answer\AnswerVoiceTextRequest;
 ...
 */
class AnswerVoiceTextController extends Controller
{

    public $prefix = "answers/voices/";

    /**
     * Create Voice & Text Answer
     *
     * Bu servis soru tipi ses & metin  ise kullanılması gereken servistir.
     *
     * @requestMediaType multipart/form-data
     *
     */
    public function create(AnswerVoiceTextRequest $request)
    {
        ...
        if ($question->option_type !== QuestionTypes::TEXT_VOICE->value) {
            abort(response()->json([
                "message" => "Sadece ses & metin tipindeki sorular için cevap gönderilebilir."
            ], 400));
        }
     ...
     *
     */
    public function update(AnswerVoiceTextRequest $request)
    {
        $userId = null;
        $questionId = $request->id;
        if (!$userId = @(Auth::guard('user-api')->user())->id) {
